...
if (heterogeneity != Heterogeneity$RANDOM) {
    ...
    params_oneshot <- bca_oneshot(sumstats, family_alt, use_local_intercepts_bca, use_local_variances,
        glm_prior_lamda = 0.1
    ) # setting 0 equivalent to FE in normal model
...
coef_list_with_sigma2 <- lapply(seq_along(coef_list), function(i) {
    c(coef_list[[i]], "sigma2" = sigma2_list[[i]])
})
...
fit.pda <- fit_pda(outcome, covariates,
    data_split, sites, dataname, family,
    dir = "data/pda", heterogeneity_effect = heterogeneity
)
```
```{r}
#| vscode: {languageId: r}
...
if (heterogeneity != Heterogeneity$RANDOM) {
    ...
    df_pda <- tidy_pda(fit.pda, family,
        covariates, n_sites, n_data,
        alpha = 0.05, heterogeneity_effect = heterogeneity
    )
}
...
if (heterogeneity != Heterogeneity$NONE) {
    fit.pda.re <- fit_pda(outcome, covariates,
        data_split, sites, dataname, family,
        dir = "data/pda", heterogeneity_effect = "random"
    )
    df_pda_re <- tidy_pda(fit.pda.re, family,
        covariates, n_sites, n_data,
        alpha = 0.05, heterogeneity_effect = "random"
    )
}
```
...
if (heterogeneity != Heterogeneity$NONE) {
    ...
    df_wide <- dfi |>
        select(Method, Covariate, Estimate) |>
        pivot_wider(
            names_from = Method,
...
box_results <- pred_check(params_oneshot, sumstats, family,
    use_local_intercepts, center_name,
...
forest_plot(df_forest, pboxes,
    outfile = paste0("figures_tmp/forest.", dataname, ".pdf"),
